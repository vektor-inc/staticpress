	...
	private function update_url( $urls ) {
		...
		foreach ( (array) $urls as $url ) {
			...
			if ( preg_match( '#\.php$#i', $url['url'] ) ) {
				$url['enable'] = 0;
			} elseif ( preg_match( '#\?[^=]+[=]?#i', $url['url'] ) ) {
				$url['enable'] = 0;
			} elseif ( preg_match( '#/wp-admin/$#i', $url['url'] ) ) {
				$url['enable'] = 0;
			} elseif ( isset( $url['type'] ) && $url['type'] == 'static_file' ) {
				...
				if ( $file_source === $file_dest ) {
					$url['enable'] = 0;
				} elseif ( preg_match( $pattern, $url['url'] ) ) {
					$url['enable'] = 0;
				} elseif ( ! file_exists( $file_source ) ) {
					$url['enable'] = 0;
				} elseif ( ! file_exists( $file_dest ) ) {
					$url['enable'] = 1;
				...
				if ( $url['enable'] == 1 ) {
					...
					} elseif ( preg_match( '#^' . preg_quote( $theme_dir ) . '#i', $url['url'] ) ) {
						$url['enable'] = 0;
